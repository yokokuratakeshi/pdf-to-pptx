...
def convert_edit_mode(doc, keep_bg, dpi, progress_bar):
    ...
    for page_idx, page in enumerate(doc):
        ...
        for block in text_dict.get("blocks", []):
            if block.get("type") != 0:  # 0 = テキストブロック
                continue
            ...
            for line in block.get("lines", []):
                ...
                for span in line_spans:
                    ...
                    try:
                        ...
                        font.color.rgb = color_int_to_rgb(color_int)
                    except Exception:
                        pass
                    ...
                    # フォント名
                    try:
                        cleaned = clean_font_name(span.get("font", ""))
                        font.name = cleaned
                    except Exception:
                        pass
    ...
    if st.button(btn_label, type="primary"):
        try:
            ...
            if not is_edit_mode:
                ...
                _keep_bg = keep_bg if 'keep_bg' in dir() else False
                prs = convert_edit_mode(doc, _keep_bg, dpi, progress_bar)
            ...
            if is_edit_mode:
                st.caption("💡 PowerPointで開いた後、テキストをクリックすると編集できます。画像はクリック→ドラッグで移動できます。")
